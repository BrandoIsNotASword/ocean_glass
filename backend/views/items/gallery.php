...
use yii\helpers\Html;
 ...
 <div class="modal-body text-center">
 	<div class="row" id='testsort' data-url='<?= Url::to(['items/updatepositions','id'=>$item->id]) ?>'>
 		<?php foreach ($item->images as $i): ?>
 		<div class="col-md-3 text-center" data-img='<?= $i->id ?>' style='margin-bottom:10px;'>
			<img class='imgZoom' style='border:1px solid grey' src="<?= Yii::$app->controller->getUrlImg($i,2) ?>" data-zoom-image='<?= Yii::$app->controller->getUrlImg($i) ?>'>
			<div style='' class="text-center">
				<a title='Selecciona como imagen principal' href='<?= Url::to(['images/principal','id'=>$i->id]) ?>' data-loading-text="<i class='icon-refresh icon-spin'></i>" class='ajaxLink btn btn-warning btn-xs'><i class='<?= $i->principal?'icon-star':'icon-star-empty' ?>'></i></a>
 	...
 	<form method='post' class='validateForm ajaxSubmit' action='<?= Url::toRoute(['images/add','id'=>$item->id]) ?>' enctype='multipart/form-data'>
